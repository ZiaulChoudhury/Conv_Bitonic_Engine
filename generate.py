...
for i in range(0,layers):
    for j , k in zip(range(i+1,0,-1), range(0,i+1)):
        ...
        program += "\n\t\t\ts"+str(tx+1)+"[mod*i+j] <= min(s"+str(tx)+"[mod*i+j] , s"+str(tx)+"[mod*i+j+mod/2]);"
        program += "\n\t\t\ts"+str(tx+1)+"[mod*i+j+mod/2] <= max(s"+str(tx)+"[mod*i+j] , s"+str(tx)+"[mod*i+j+mod/2]);"
        program += "\n\t\t\tend"
        program += "\n\t\tend"
        program += "\n\t\telse begin"
        program += "\n\t\t\tfor(int j=0; j< mod/2; j=j+1) begin"
        program += "\n\t\t\ts" + str(tx + 1) + "[mod*i+j] <= max(s" + str(tx) + "[mod*i+j] , s" + str(tx) + "[mod*i+j+mod/2]);"
        program += "\n\t\t\ts" + str(tx + 1) + "[mod*i+j+mod/2] <= min(s" + str(tx) + "[mod*i+j] , s" + str(tx) + "[mod*i+j+mod/2]);"
        program += "\n\t\t\tend"
        program += "\n\t\tend"
        program += "\n\tend"
        program += "\n\tp"+str(tx+1)+".enq(1);"
