...
for i in range(0,layers):
    for j , k in zip(range(i+1,0,-1), range(0,i+1)):
        ...
        for j in range(0,mod):
            program += "\n\t\t\t Vector#(2, Int#(16)) _S"+str(tx+1)+"i"+str(j)+" = min_max(pack(s"+str(tx)+"[mod*i+"+str(j)+"]) , pack(s"+str(tx)+"[mod*i+"+str(j)+"+mod/2]));"
...
for i in range(0,seq_length):
    ...
    convID  = generate_random_16bit_number(1,4)
    value   = generate_random_16bit_number(10,100)
    payload = (value << 8 )|((convID << 4) | partitionID)
